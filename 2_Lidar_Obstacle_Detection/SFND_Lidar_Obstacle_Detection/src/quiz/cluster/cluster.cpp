...
std::vector<int> euclideanClusterHelper(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol, int indice, std::vector<bool> &processed, std::vector<int> &cluster)
{
	processed[indice] = true;
	cluster.push_back(indice);
	std::vector<int> nearest_points = tree->search(points[indice], distanceTol);
	for (int id : nearest_points)
	{
		if (!processed[id])

		{
			euclideanClusterHelper(points, tree, distanceTol, id, processed, cluster);
		}
	}
}
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (i = 0; i < points.size(); i++)
	{
		...
		else
		{
			...
			euclideanClusterHelper(points, tree, distanceTol, i, processed, cluster);
