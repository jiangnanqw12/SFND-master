...
=======
  ...
  void insertHelper(Node **nodePP, uint depth, std::vector<float> point,
                    int id)
  {
    // if tree is empty
    if (*nodePP == NULL)
  ...
  std::vector<int> searchHelper(std::vector<float> target, float distanceTol,
                                std::vector<int> &ids, int depth, Node *node)
  {
    // float x0 = target[0];
    // float y0 = target[1];
    // float x = node->point[0];
    ...
    if (node != NULL)
    {
      ...
      // x<= left boundary ,the left of select point wont have chance to be in
      // the boxes
      if (node->point[depth % 2] > target[depth % 2] - distanceTol)
      {
        searchHelper(target, distanceTol, ids, depth + 1, node->left);
...
>>>>>>> 1e4a8a4bc8221307d7e535794e8e9e3f86a26d08
};
